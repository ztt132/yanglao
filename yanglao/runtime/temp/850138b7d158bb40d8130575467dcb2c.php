<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"/Users/cesc/365web/yanglao/yanglao/public/../application/admin/view/photo/index.html";i:1592980490;}*/ ?>
<!DOCTYPE html>
        ...
        <div class="layui-col-md12">
            <div class="layui-card padding20">
                ...
                <div class="layui-card-header">
                    <button class="layui-btn" onclick="xadmin.open('添加相册','/admin/photo/add',600,700)"><i class="layui-icon"></i>添加</button>
                </div>
...
<script>
    layui.use(['table','form'], function(){
        ...
        var districtTable = table.render({
            elem: '#list',
            url: '/admin/photo/photoList', //数据接口
            page: true, //开启分页
            method:'get',
            ...
            cols: [[
                ...
                {field: 'org_name', title: '机构名称', align:'center', sort: false, fixed: 'left'},
                {field: 'pic', title: '图片', align:'center', templet:'#img', fixed: 'left'},
                {field: 'photo_type', title: '图片类型',align:'center',  sort: false, fixed: 'left'},
                {field: '操作', title: '操作',align:'center',  templet: '#operation', fixed: 'left'},
                {field: 'is_cover', title: '选为封面', align:'center', templet: '#cover', fixed: 'left'}
            ]]
        });
    ...
    function edit(obj) {
        ...
        layui.use('layer',function(){
            ...
            layer.open({
                ...
                type:2,
                area: ['50%', '80%'],
                content:'/admin/photo/edit?id='+id
            })
        })
    }
</script>
...
<script type="text/html" id="cover">
    {{#  if(d.is_cover == 1){ }}
    <input val="{{d.id}}" checked type="checkbox" name="is_cover" title="封面" lay-filter="is_cover" value="{{d.is_cover}}" lay-skin="switch">
    {{#  } else { }}
    <input val="{{d.id}}" type="checkbox" name="is_cover" title="封面" lay-filter="is_cover" value="{{d.is_cover}}" lay-skin="switch">
    {{#  } }}
